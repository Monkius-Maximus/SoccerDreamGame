...
public partial class LifeSimScene : Node2D
{
    ...
    {
        IEnumerable<string> parts = outcome.AppliedDeltas
            .Select(delta => $"{(delta.Delta >= 0 ? "+" : "")}{delta.Delta:0} {delta.Need}");

        string summary = string.Join("  ", parts);
        _log.Text = summary.Length == 0
            ? $"{outcome.ActivityKey}: no change — those needs were already full."
            : $"{outcome.ActivityKey}: {summary}";
    }
}
